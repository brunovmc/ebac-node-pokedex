...
const doCaptureAnimation = () => {
  const { pokemon, pokeball, captureButton } = htmlElements();
...
const makeCaptureRequest = () => {
  ...
  setTimeout(() => {
    if (response.capturado) {
      const congratulationsMessage = encodeURIComponent(
        "Parabéns!! Pokemon capturado"
      );
...
document.addEventListener(
  ...
  function (event) {
    if (
      !event.target.matches(".capture-pokemon") ||
      event.target.matches(".loading")
    )
      return;
